<script>
	...
	onMount(async () => {
		...
		relatedGamesArr = res.data.list.slice(0, 6);
	});

	function relatedGamesSliderPrev() {
		relatedGamesSlider.slidePrev();
	}

	function relatedGamesSliderNext() {
		// swiper.slidePrev();
		relatedGamesSlider.slideNext();
	}
</script>
	...
	<div class="w-full flex items-center justify-between">
		...
		<div class="flex items-center gap-[11px]">
			<button
				...
				id="slidePrev"
				on:click={relatedGamesSliderPrev}
			>
			...
			<button
				class="bg-grayLight dark:bg-blueDark border border-grayLight4 dark:border-white11 opacity-80 hover:opacity-100 rounded-[11px] w-[36px] h-[36px] flex items-center justify-center"
				id="slidePrev"
				on:click={relatedGamesSliderNext}
			>
