...
<script>
	...
	import { changeProfileImage, changeNickname, getAccessToken, getRefreshToken } from '../../../../apis/account';
  ...
  let cropper;
	...
	async function handleMyProfile(event) {
		...
		const res = await changeProfileImage({
			memberIdx: $globalStore.userDetail.member_idx,
...
<ModalLayout on:stepBack={stepBack} step={step} on:closeProfileModal={closeProfileModal} title="Edit Profile">
	<form on:submit={handleMyProfile}>
		<div class="flex justify-center w-full h-[200px] rounded-[14px] bg-gray preview mt-[24px] ">
			...
			<div bind:this={container} class="h-[200px] flex items-center">
				...
				{:else}
					<span class="cursor-pointer text-black dark:text-grayLight" bind:this={placeholder} on:click={() => {
    ...
    <button class="relative z-[99] w-full py-[14px] bg-linear rounded-[7px] mt-[30px] text-white font-medium opacity-80 hover:opacity-100 transition-all">Save</button>
	</form>
</ModalLayout>
