<script>
	...
	let dynamicMainSportsItems = mainSportsItems; // use dynamicMainSportsItems as dynamicArray
    // currentTab must be same as mainSportsTab title
    let currentTab = 'SoCCer' // you can add as uppercase as lowercase chars because in filter when compare to string to each other both string willl be transform to lowerCase letters
    function handleFilter(title) {
        currentTab = title.toLowerCase()
        updateDynamicMainSportsItems();
    }
    function updateDynamicMainSportsItems() {
        dynamicMainSportsItems = mainSportsItems.filter(item => {
...
<div class="w-full flex flex-col items-start gap-[15px] mt-[37px]">
	<div class="w-full flex items-center justify-between">
		<h6 class="font-semibold text-xl gradient-text-white2 main">Main Sports</h6>
	</div>
	<div class="w-full flex flex-wrap items-center gap-[15px]">
		{#each mainSportsTab as tab}
		<div
				on:click={() => handleFilter(tab.title)}
				class={`${
					currentTab.toLowerCase() === tab.title.toLowerCase() ? 'bg-linear active-p-white' : ''
				} ${tab.hide ? 'hidden sm:flex' : 'flex'} group items-center gap-[8px] p-[12px] bg-black5 dark:bg-white5 border border-black5 dark:border-white11 rounded-[7px] cursor-pointer hover:bg-linear`}
			>
				<img class="w-[15px] mb-[3px]" src={tab.image} alt={tab.title} />
				<p class="group-hover:text-white text-black dark:text-white text-xs sm:text-sm font-normal uppercase">{tab.title}</p>
			</div>
		{/each}

		<a href="/" class="flex sm:hidden ml-auto gradient-text hover:text-blue transition-all font-normal text-sm">See All</a>

	</div>
	<div class="w-full flex flex-col gap-[15px] mt-[5px]">
		{#each dynamicMainSportsItems as item}
		<div class="w-full bg-white dark:bg-blackDark px-[13px] pt-[13px] pb-[11px] rounded-[7px]">
			<div class="w-full grid gap-[10px] sm:gap-[15px] md:gap-[0px] md:grid-cols-[450px,1fr]">
				<div class="flex items-center">
					...
					>
						<p class="min-w-max text-xs sm:text-base font-medium text-grayDark dark:text-grayLight">{item.timeData.time}</p>
						<p class="min-w-max text-xs sm:text-base font-medium text-black dark:text-grayDark2">{item.timeData.date}</p>
					</div>
					<div class="flex flex-col gap-[12px] sm:gap-[20px]">
						...
						<div class="flex items-center gap-[15px] sm:gap-[31px]">
							<div class="flex items-center gap-[6px] sm:gap-[10px]">
								<img class="w-[22px] sm:w-[27px]" src={item.teams.team1.image} alt={item.teams.team1.title} />
								<p class="text-sm sm:text-base font-bold text-black dark:text-grayLight">{item.teams.team1.title}</p>
							</div>
							<p class="text-sm font-normal text-grayDark2">VS</p>
							<div class="flex items-center gap-[6px] sm:gap-[10px]">
								<img class="w-[22px] sm:w-[27px]" src={item.teams.team2.image} alt={item.teams.team2.title} />
								<p class="text-sm sm:text-base font-bold text-black dark:text-grayLight">{item.teams.team2.title}</p>
							</div>
						</div>
					</div>
				</div>
				...
				>
					...
					<div class="flex flex-col gap-[5px] items-center">
						<p class="text-xs sm:text-sm font-bold text-grayDark2">HANDICAP</p>
						<div class="flex items-center gap-[5px]">
							<div
							...
							>
								...
								<p class="text-sm sm:text-base font-medium text-black dark:text-white">{item.handicap.first.title}</p>
							</div>
							<div
							class="flex flex-col items-center justify-center rounded-[7px] text-black dark:text-white cursor-pointer hover:border-blue text-sm sm:text-base w-[65px] sm:w-[81px] h-[52px] bg-grayLight4 dark:bg-white5 border border-grayLight2 dark:border-white11"
							>
								<p class="text-xs font-medium text-red">{item.handicap.second.subTitle}</p>
								<p class="text-sm sm:text-base font-medium text-black dark:text-white">{item.handicap.second.title}</p>
							</div>
						</div>
					</div>
					<div class="flex flex-col gap-[5px] items-center">
						<p class="text-sm font-bold text-grayDark2">TOTAL</p>
						<div class="flex items-center gap-[5px]">
							{#each item.total as totalItem}
								...
								>
									...
									<p class="text-sm sm:text-base font-medium text-black dark:text-white">{totalItem.title}</p>
								</div>
							{/each}
						</div>
					</div>
