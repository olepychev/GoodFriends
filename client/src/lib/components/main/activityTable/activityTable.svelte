...
<div class="w-full flex flex-col items-start gap-[15px] mt-[37px]">
	...
	<div class="relative w-full">
		<ul class="flex md:flex-wrap md:justify-start justify-between items-center gap-[12px] sm:gap-[40px] mt-[12px] sm:mt-[20px]">
			<li on:click={()=> handleFilter(1)}
				class={`${activeFilter == 1 ? 'active' : ''} gradient-text text-[10px] sm:text-xs font-semibold uppercase pb-[14px] cursor-pointer hover-gradient-text transition-all`}
			>
				Latest Bets
			</li>
			<li on:click={()=> handleFilter(2)}
				class={`${activeFilter == 2 ? 'active' : ''} ${!showCols[1] ? 'hidden' : ''} gradient-text text-[10px] sm:text-xs font-semibold uppercase pb-[14px] cursor-pointer hover-gradient-text transition-all`}
			>
				Recent Deposits
			</li>
			<li on:click={()=> handleFilter(3)}
				class={`${activeFilter == 3 ? 'active' : ''} ${!showCols[2] ? 'hidden' : ''} gradient-text text-[10px] sm:text-xs font-semibold uppercase pb-[14px] cursor-pointer hover-gradient-text transition-all`}
			>
				Recent Withdrawals
			</li>
		</ul>
	...
	<div class="w-full mt-[20px] flex flex-col cursor-dragable overflow-auto dragable overflow-auto scrollbar-hidden">
		<div class="w-full min-w-[755px] gap-[15px] grid grid-cols-5 rounded-[14px] bg-grayLight2 dark:bg-black21 px-[28px] py-[20px]">
			<p class="text-msm md:text-sm lg:text-base font-medium text-black dark:text-white">Game</p>
			<p class="text-msm md:text-sm lg:text-base font-medium text-black dark:text-white">Player</p>
			<p class="text-msm md:text-sm lg:text-base font-medium text-black dark:text-white">Bet Amount</p>
			<p class="text-msm md:text-sm lg:text-base font-medium text-black dark:text-white">Multiplier</p>
			<p class="text-end text-msm md:text-sm lg:text-base font-medium text-black dark:text-white">Profit Amount</p>
		</div>
