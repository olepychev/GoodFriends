<script>
	...
	let dynamicLiveSportsItems = liveSportsItems; // use dynamicLiveSportsItems as dynamicArray
	// currentTab must be same as liveSportsTabs title
	let currentTab = 'esports'; // you can add as uppercase as lowercase chars because in filter when compare to string to each other both string willl be transform to lowerCase letters
	function handleFilter(title) {
		currentTab = title.toLowerCase();
		updateDynamicSportsItems();
	}
	function updateDynamicSportsItems() {
		dynamicLiveSportsItems = liveSportsItems.filter((item) => {
...
<div class="w-full flex flex-col items-start gap-[15px] mt-[37px]">
	<div class="w-full flex items-center justify-between">
		<h6 class="font-semibold text-xl gradient-text-white2 main">Live Sports</h6>
	</div>
	<div class="w-full flex flex-wrap items-center gap-[15px]">
		{#each liveSportsTabs as tab}
			<div
	...
	<div class="w-full flex flex-col gap-[15px] mt-[5px]">
		{#each dynamicLiveSportsItems as item}
